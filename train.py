...
#------------------------------------ METRIQUES AVEC MLFLOW---------------------------------#
    ...
    class_names = list(df["Sentiment"].unique())  # Récupérer les classes dynamiquement
    ...
    mlflow.log_artifact("matrice_de_confusion.png")

    # Création d'une pipeline combinant prétraitement et modèle
    full_pipeline = Pipeline([
        ('tfidf', vectorizer),  # vectorizer TF-IDF
        ('clf', model)          #  modèle LogisticRegression
    ])
    # Convertir X_train en DataFrame (si X_train est une matrice creuse)
    X_train_df = pd.DataFrame(X_train.toarray(), columns=vectorizer.get_feature_names_out())

    # Exemple d'entrée pour MLflow
    input_example = X_train_df.iloc[:1]  # Prendre la première ligne comme exemple
    signature = infer_signature(X_train, model.predict(X_train))

    # Sauvegarde avec MLflow

    mlflow.sklearn.log_model(full_pipeline, "sentiment_analysis_pipeline", input_example=input_example, 
                               signature=signature)
